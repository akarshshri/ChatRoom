...
function getAnno() {
	...
	if(a !== ''){
			...
			xhr = new XMLHttpRequest();
			xhr.open('POST' , 'anno_football.php',true);
			xhr.setRequestHeader('content-type','application/x-www-form-urlencoded');
			xhr.send('anno='+$a);
			xhr.onreadystatechange=function(){
				if (xhr.responseText){
...
function setText2(){
	...
	xhr = new XMLHttpRequest();
	xhr.open('POST' , 'footballchatFetch.php' , true);
	xhr.setRequestHeader('content-type','application/x-www-form-urlencoded');
	xhr.send();
	xhr.onreadystatechange = function(){
	...
	xhr2.open('POST' , 'footballannoFetch.php' , true);
...
include_once('config.php');
			...
			echo '<center><img src="images/football.png" height="50px" style="margin-top:1%;"></center><hr>';
				//	echo		$_SESSION['email'];
